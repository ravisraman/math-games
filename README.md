...
**Sync laptop ⇄ iPhone:** on one device open ⚙️ Grown-ups → **Turn on sync** to get a family code,
then on the other device (inside the Home Screen app on iPhone) open ⚙️ → type the code → **Join**.
After that, stars, levels, heroes and settings stay in step automatically. Progress from both devices
is merged (stars earned on each are added up), and each device keeps working offline.
...
Questions, hints and praise are read aloud in a natural neural voice (Deepgram Aura-2 for English,
MeloTTS for Chinese) generated by the family's Cloudflare Worker (`cloud/worker.js`); each sentence
is made once and then reused. Grown-ups can pick the voice or switch it off. When offline, or once the
free daily voice allowance is used up, the best voice installed on the device is used instead
(download a free **Premium** voice in the device's Spoken Content settings for the best result).
...
cloud/worker.js                     Cloudflare Worker: progress sync (KV) + natural voice (Workers AI)
```
